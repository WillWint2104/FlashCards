...
(function () {
  ...
  function esRestHTML(p) {
    ...
    if (esIsIntro(p) || esIsConcl(p)) {
      ...
      // Body paragraphs rest empty: everything the rail used to hold for them is a
      // tool one press away. The introduction and the conclusion keep theirs,
      // because it carries the judgement the student took and what their paragraphs
      // established, and neither has another home. Emptying it there would remove
      // the only view of it, which is a capability loss rather than a tidy up.
      if (!ES.ui.tool && !intro && !judging) {
