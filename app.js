...
(function () {
  ...
  // Nudges are kept as written (questions, not answers). Chips are WORD-LEVEL only:
  // any "from" or option longer than a short phrase is dropped, so the coach can
  // never return a paste-ready sentence or paragraph through this channel.
  ...
  function esNormalizeCoach(raw, demoNote) {
    ...
    const chips = (Array.isArray(raw.chips) ? raw.chips : [])
      ...
      .slice(0, 6);
    return { note: String(raw.note || "").trim(), nudges, chips, check: String(raw.check || "").trim(), demoNote: demoNote || "" };
  }
  ...
  function esRenderSetup(host, sc) {
    ...
    const resume = ES.list.length ? `
      <div class="es-resume">
        ...
        <div class="es-resrow">${ES.list.map(d =>
          `<button class="es-rescard" data-esresume="${esc(d.id)}">
    ...
    host.querySelectorAll("[data-esresume]").forEach(b => b.onclick = () => {
      ...
      if (d) { ES.draft = d; ES.screen = d.mode === "full" ? "full" : "coached"; esRender(); }
    });
    $("#esstart").onclick = () => {
  ...
  function esApplyChip(idx, from, option) {
    ...
    const ta = $("#espara"); if (ta) { ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length); }
  }
  ...
  function maybeOpenEssayDemo() {
    if (gated()) return;
    try {
      if (/[?&]essaydemo=1/.test(location.search)) esOpen({ demo: true, subject: "ancient_history", code: "11Anc1" });
      else if (essayEnabled() && /[?&]essay=1/.test(location.search)) esOpen({});
    } catch (e) { /* demo entry is best-effort */ }
  }
