...
(function () {
  ...
  // stays content-only.
  // Trimmed on both sides. The label reaching here can come from an imported
  // paper's JSON, which is authored text, and " Business Studies " matching
  // nothing meant markingContext fell through to C.markingCriteria: a Business
  // Studies answer marked against the app's own Economics criteria, with nothing
  // on screen to say so. Trimming closes that without changing what matches.
  ...
  // WHICH DOOR A PASTED FILE GOES THROUGH, decided by the schema FAMILY and not
  // by an exact version match.
  //
  // The routing was `data.format === EXAM_FORMAT`, so a package declaring
  // marginal-exam@2 failed that equality, fell through to the flashcard-set
  // validator, and the person holding an exam file was told "The set has no
  // cards array." The exam contract has a correct answer for that package -
  // PACKAGE_VERSION_UNSUPPORTED, naming the version it does run - and it was
  // unreachable, because nothing ever handed it the file.
  //
  // Recognising the family is not accepting the version. It only means the exam
  // validator gets to be the one that refuses it, and to refuse it for the real
  // reason.
  const EXAM_FAMILY = /^marginal-exam(@|$)/i;
  function looksLikeExam(d) {
    return !!(d && typeof d.format === "string" && EXAM_FAMILY.test(d.format.trim()));
  }
  ...
  function importSet() {
    ...
    // A whole paper, not a flat set. Any version of the family comes here; the
    // exam contract decides whether this release can run it.
    if (looksLikeExam(data)) return importExamFromBox(data, msg);
  ...
  // Whether the application is in a subject it can show a student anything about.
  // A class code routing to a subject whose content is not written yet leaves
  // ES.subject holding a key nothing resolves. That is supported, and it is not
  // the same thing as the student being in that subject.
  function esHasSubject() { return !!esSubjectContent(ES.subject); }
  function esSubjectLabel() {
    const sc = esSubjectContent(ES.subject);
    if (sc && sc.label) return sc.label;
    // A key with no content behind it is not a subject to name. Humanising it
    // produced "Economics" in the header of an application that has never held a
    // single economics question, which is a label invented from a routing rule
    // rather than from anything a student could open.
    return "";
  }
  ...
  // demoKind says WHY this is not live feedback, because the three reasons are not
  // the same thing to a student and must not look the same:
  //   ""            live feedback on their own writing
  ...
  function esOpen(opts) {
    opts = opts || {};
    ES.demo = !!opts.demo;
    // A routing rule may name a subject whose content is not written yet, and
    // that is a supported state: the core works for every login and only the
    // subject-specific add-ons are missing. What is NOT supported is a screen
    // claiming the student is in a subject when they are not. See esSubjectLabel
    // and the picker, which is where the two disagreeing labels came from.
  ...
  function esRenderUnavailable(host) {
    ...
    <div class="es-scrim"><div class="es-shell"><div class="es-wrap">
      ...
      <div class="es-top"><div class="qp-brand static"><span class="qp-logo" aria-hidden="true">M</span><span class="qp-word">Marginal</span></div>
        <div class="es-topbtns"><button type="button" class="es-util quiet" id="esx" aria-label="Marginal home">${esIcon("home")}<span>Home</span></button></div></div>
    ...
    const x = $("#esx"); if (x) x.onclick = esLeave;
  }
  ...
  // WHAT THIS QUESTION ACTUALLY OFFERS, read off the question and nothing else.
  // Every line is a fact about what somebody authored: a question with no
  // pathways has no pathway guidance, and saying so before a student commits is
  // the honest version of finding out afterwards. Nothing here is a capability
  // score and nothing is borrowed from another question.
  function esSupportFor(q) {
    const paths = (q && q.pathways) || [];
    const areas = Object.keys((q && q.areas) || {});
    const rows = [
      { key: "marking", label: "Marking guidance", has: true,
        note: "Written for this question, or generated from it" },
      { key: "planning", label: "Planning support", has: areas.length > 0,
        note: areas.length ? areas.length + " areas to plan across" : "This question defines no areas" },
      { key: "pathway", label: "Pathway guidance", has: paths.length > 0,
        note: paths.length ? paths.length + " arguments to choose between" : "This question authors no arguments" },
      { key: "learning", label: "Learning support", has: paths.some(p => (p.learning || {}).status === "authored"),
        note: "A lesson for a student who does not know the content yet" },
      { key: "evidence", label: "Evidence support", has: paths.some(p => (p.evidenceRefs || []).length),
        note: "Evidence attached to the arguments" },
    ];
    const have = rows.filter(r => r.has);
    return { rows: rows,
      // The one line summary the preview leads with. It names what IS there.
      summary: have.length === 1 ? have[0].label.toLowerCase() + " only"
        : have.map(r => r.label.toLowerCase()).join(", "),
  ...
  function esRenderSetup(host, sc) {
    ...
    // student has no committed subject, and at "> 1" a build with a single
    // registered subject would leave them no way to reach one at all.
    const subjectPicker = subjectList.length >= 1 ? `
      <div class="qp-field">
        ...
        ${/* A <select> with no option marked shows its FIRST one, and that is how
              this control came to display "Ancient History" while the header said
              "Economics": the routed subject was in neither the list nor the
    ...
    host.querySelectorAll("[data-espick]").forEach(b => b.onclick = () => {
      ...
      else { f.setupMode = "practice"; f.pickStage = to; }
      esRender();
    });
    const bl = $("#esbacklist"); if (bl) bl.onclick = () => { f.pickStage = "list"; esRender(); };
    ...
    // Same guard: the structure picker is inside the folded options, which the
    // list stage does not render at all.
    const stt = $("#esstruct"); if (stt) stt.onchange = () => { f.structure = stt.value; };
    const subjSel = $("#essubject");
    if (subjSel) subjSel.onchange = () => {
      if (!subjSel.value) return;              // the "Choose a subject" placeholder
    ...
    // which left every binding after it unattached and the rows dead.
    const startBtn = $("#esstart");
  ...
  function esNavLeftHTML(current) {
  ...
  function esFeedbackStale(p) {
    ...
    if ((p.gradedText || "") !== (p.text || "")) return true;
    // The words can be identical and the check still out of date: the paragraph now
    // argues something else. Guidance is resolved through the current pathway, so a
    // result obtained under the previous one is not about this paragraph any more.
    return esContextChanged(p);
  }
  // The paragraph's argument or evidence changed after it was checked. Old results
  // that predate the recording carry no contextVersion and are not called stale on
  // that account: a missing field is not evidence of a change.
  function esContextChanged(p) {
    const snap = p && p.feedback && p.feedback.checked;
    if (!snap || snap.contextVersion === undefined) return false;
    return (Number(p.contextVersion) || 0) !== (Number(snap.contextVersion) || 0);
  }
  ...
  const ES_ICON_SRC = {
    ...
    // something did not work, and the student needs to know before they read on
    ...
    // leave Essay Practice for Marginal's home
  ...
  function esReviewHTML(p) {
    ...
    const staleBar = stale ? `<div class="es-rstale">${esIcon("info")}<span>${esc(esContextChanged(p)
      ? "This paragraph argues something different from when it was checked, so what follows was written about the previous argument. Re-check it."
      : "This is the check of the previous version of your paragraph. Re-check when you have finished editing.")}</span></div>` : "";
    ...
    // Stale, or a refused result there is a point in asking again for.
    const canRecheck = stale || !!refused;
    return `<div class="es-review">
      ...
      ${/* RE-CHECK IS FOR A PARAGRAPH THAT HAS MOVED ON. It used to be enabled the
             moment a result arrived, so the primary action on a review the student
             had not touched was to spend another worker call asking the same
             question about the same words. It is offered when the paragraph no
             longer matches what was checked, and when a result came back unusable
             and there is genuinely something to retry. */ ""}
      <div class="es-ractions">
        <button type="button" class="es-btn primary sm" id="esrecheck"${
          canRecheck ? "" : " disabled title=\"Nothing has changed since this check\""
        }>${esIcon("feedback")}<span>Re-check paragraph</span></button>
        ${openCount > 1
  ...
  function esReviewRowHTML(p, r) {
    ...
    return `<div class="es-rbody">
      ...
      <div class="es-rrow">
        ...
        <span class="es-sp"></span>
        ${/* SAVE IS FOR A REVISION. The box opens holding the student's own sentence,
               so on a review nobody has typed into, Save writes the sentence back
               over itself - which changes gradedText, makes the feedback stale and
               manufactures a re-check out of no edit at all. It turns on when the
               box stops matching the sentence it started from. */ ""}
        <button type="button" class="es-btn primary sm" id="esrsave" disabled>${esIcon("save")}<span>Save revision</span></button>
      </div>
  ...
  //
  // AND THERE IS NO FALLBACK, which is the correction. The subject's slot template
  // was used when the pathway authored nothing, on the reasoning that a template
  // authored for that slot's job in that subject is better than an empty section.
  // It is not, and the exhibit proved it: the template above is what 27 of the 28
  // Explain pathways were showing under the characteristic-to-strategy diagnosis
  // above, because only one pathway in the whole subject authors a frame.
  //
  // A scaffold in Paragraph Review is not decoration next to a diagnosis: it is the
  // answer to it, and the student writes their sentence from it. One that teaches a
  // different causal job than the one just diagnosed is worse than no scaffold at
  // all. So the review shows the pathway's frame for this slot, on this question,
  // under this directive family, or it shows no "try this structure" section and
  // the student gets diagnosis, revision editor and More help.
  //
  // THIS IS THE REVIEW'S RULE ONLY. esSlotFrame has exactly one caller, the review
  // row; the sentence composer resolves its own frames through slotTemplates at
  // esSlotGuide and keeps every fallback it had.
  function esSlotFrame(p, key) {
    ...
    return authored ? { text: String(authored.text || authored), source: "pathway" } : null;
  }
  ...
  function esCoachMargin(p) {
    ...
    const fbk = (p.feedback && p.feedback.demoKind) || "";
    // The panel says what it is holding. Feedback about the student's own
    // paragraph and demo guidance that is not about it are not the same panel
    // with a different sentence at the top.
    const head = `<div class="es-mhead">${esIcon("feedback")}<span>Coach feedback</span>${
      fbk ? `<span class="es-mtag ${fbk === "unreachable" ? "warn" : ""}">${
        fbk === "unreachable" ? "not your feedback" : "demo guidance"}</span>` : ""}</div>`;
    ...
    // A failed call is a warning, not a completion. Green is the colour this app
    // uses for something having gone right, and a coach that could not be reached
    // wearing it told the student the opposite of what happened.
    const demo = fb.demoNote
  ...
  function esBindReview(p, host) {
    ...
    // SAVE FOLLOWS THE BOX. Rendered disabled, and turned on only while the text in
    // it differs from the sentence it opened holding. An empty box is never
    // saveable, whether the slot had a sentence or not.
    const svBox = host.querySelector("[data-esrbox]");
    const sv = host.querySelector("#esrsave");
    if (sv && svBox) {
      const start = String(svBox.value || "");
      const sync = () => {
        const now = String(svBox.value || "");
        sv.disabled = !now.trim() || now.trim() === start.trim();
      };
      svBox.oninput = sync;
      sync();
    }
    if (sv) sv.onclick = () => {
      const box = svBox || host.querySelector("[data-esrbox]");
      if (!box) return;
  ...
  async function esGetFeedback(idx) {
    ...
    fb.checked = {
      text: submittedText,
      // WHICH ARGUMENT IT WAS CHECKED UNDER. A student can change a paragraph's
      // argument after writing it, and the app already flags every written sentence
      // when they do. The review read none of that: the diagnosis, the scaffold and
      // More help all resolve through the paragraph's CURRENT pathway, so changing
      // the argument silently re-pointed the guidance at prose written for the old
      // one. Recorded here so the review can tell that the check predates the change.
      contextVersion: Number(p.contextVersion) || 0,
    ...
    // A RESULT IS A MODE CHANGE, so this renders rather than patching. What follows
    // the render is placement, not content: the sentence under the open row is
    // marked and the panel is brought into view once.
    if (ES.screen === "coached" && ES.draft && ES.draft.pos === idx) {
      // The partial update existed so nothing flashed, and it was right while the
      // review was a panel under the writer. Now that arriving feedback stands the
      // composer down, updating only the margin left the old writing surface on
      ...
      // The lines block, the ask button, the stepper's done mark and the sequencing
      // nudge were all patched in here, one by one, because there was no render to
      // draw them. There is now, and it draws every one of them from the same state
      // this code was reaching in to correct. Patching them again after it was not a
      // belt and braces: esRefreshAskButton was the only thing restoring the button's
      // label, so with a render above it that guard stopped being reachable and the
      // mutation for it stopped being able to fail.
    } else {
