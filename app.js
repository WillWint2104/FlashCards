...
(function () {
  ...
  function esBuildParas(structureKey, prev) {
    ...
    return st.roles.map((role, i) => {
      ...
      return { role, point: old ? old.point : "", text: old ? old.text : "", feedback: old ? old.feedback || null : null, gradedText: old ? old.gradedText || null : null, mastered: old ? !!old.mastered : false, blocks: null, blocksFrom: null, step: 0 };
    });
  }
  ...
  // Split a paragraph into sentences, keeping the trailing punctuation with each
  // one so joining them back is lossless.
  ...
  // The guided view of this paragraph. Rebuilt from text whenever text moved on
  // without us (a full attempt edit, a restored draft, an older saved essay), so
  // the two representations can never drift apart.
  function esBlocks(p) {
    const text = String(p.text || "");
    if (!Array.isArray(p.blocks) || p.blocksFrom !== text) {
      const steps = slotsForRole(p.role);
      p.blocks = esSplitBlocks(text).map((t, i) => ({ text: t, step: (steps[Math.min(i, steps.length - 1)] || {}).key || "" }));
      p.blocksFrom = text;
    }
    return p.blocks;
  }
  // Write the blocks back to the paragraph text. This is the ONLY place guided mode
  // touches the draft, which is what keeps the single-draft rule intact.
  function esCommitBlocks(p) {
    p.text = p.blocks.map(b => b.text.trim()).filter(Boolean).join(" ");
    p.blocksFrom = p.text;
  ...
  function esRenderCoached(host, sc) {
    ...
    if (line && accept) {
      ...
      accept.onclick = () => {
        ...
        p.blocks = esBlocks(p).concat([{ text: v, step: step ? step.key : "" }]);
        esCommitBlocks(p);
  ...
  function esReviseParagraph(idx, quote) {
    ...
    ES.ui.editBlock = p ? esBlockForQuote(p, quote) : null;
    ES.ui.rung = 0;
    esGoCoached(slot);
  ...
  // Which sentence block the marker's quote sits in, or null.
  function esBlockForQuote(p, quote) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (t.indexOf(q) >= 0 || (head.length > 8 && t.indexOf(head) >= 0) || q.indexOf(t) >= 0) return i;
    }
    return null;
  }
