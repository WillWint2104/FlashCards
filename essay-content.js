...
window.ESSAY = {
  ...
  // VOCABULARY v1
  //
  // A shared store of TERMS THAT HAVE A MEANING, and nothing else. Two rules, and
  // the whole design follows from them:
  //
  //   Nothing is generated by scanning prose. A term reaches a student because a
  //   question, an argument or an area NAMED it, the same way study resources and
  //   sentence shapes resolve. The 405 term strings sitting in the subject content
  //   are not a vocabulary list; they are words that were typed next to a point,
  //   and no one wrote down what any of them means.
  //
  //   Undefined vocabulary is never displayed. A chip carrying a word and no
  //   meaning is worse than an empty panel, because a student cannot tell the
  //   difference between a term the app is teaching and a term it merely printed.
  //   A record without all four of its fields does not resolve.
  //
  // A record is deliberately four things, not one gloss. The plain meaning is what
  // the word means in English; the subject meaning is what it means in this course,
  // which is often narrower; and the example is the term doing its job in a real
  // sentence, so a student can see the difference between knowing a definition and
  // being able to use one.
  //
  //   { id, term, plain, subject, example }
  //
  // A REF is how something asks for a term, and it carries the role that term plays
  // where it was asked for, so a panel can group by what a word is FOR rather than
  // alphabetically:
  //
  //   { id, role: "relationship-support" | "strategy-example"
  //             | "outcome-evidence"     | "topic-context" }
  //
  // Nothing here inserts anything into a student's sentence. It never has and the
  // panel has no control that could.
  //
  // EMPTY ON PURPOSE. The architecture ships with no records: authoring 405 HSC
  // definitions is content work, and the build reports the gap rather than the app
  // hiding it. Until a record exists the tool says so.
  vocab: {
    roles: [
      { id: "relationship-support", label: "for the relationship you are arguing" },
      { id: "strategy-example",     label: "naming the strategy" },
      { id: "outcome-evidence",     label: "for the outcome or the evidence" },
      { id: "topic-context",        label: "background for this topic" }
    ],
    records: {}
  },
  shapes: {
