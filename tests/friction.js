...
(async () => {
  ...
  async function doSetup(sec, argRe, evRe, useOwn) {
    if (!(await inSetup())) { note('setup', sec + ': no setup card shown'); return; }
  ...
  async function nextSection() { await p.click('#esnext'); await p.waitForTimeout(400); }
  ...
  for (let i = 0; i < BODIES.length; i++) {
    ...
    if (i === 2) {
      ...
      note(sec, 'starting Body 3, sentences of earlier writing on screen: ' + own.sentencesOnScreen);
  ...
  {
    const t0 = await since();
    // How does a student read the whole thing before submitting?
  ...
  fs.writeFileSync(OUT + 'metrics-' + PROFILE + '.json', JSON.stringify(REC, null, 1));
