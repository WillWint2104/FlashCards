...
// THE IMPORTER, as a second single file build. A teacher surface, separate from
// the student app: it shares no code with it, so nothing in it can change what a
// student sees, and the student build stays byte for byte what it was.
//
// It inlines three things. The contract bundle, which is the real modules
// wrapped to run in a browser rather than a re-implementation of their rules.
// The generated manifest and registries, because the browser cannot walk the
// content files. And importer.js, which arranges what those modules return and
// decides nothing itself.
//
// All of this is fallible: it reads six sources and evaluates the contract, so
// it belongs here, above the line, with the rest of the work that is allowed to
// refuse the build.
const importerShell = read("importer.html");
const contractBundle = require("./tools/contract/bundle.js").bundle(root);
const importerData = "// GENERATED by build.js. The manifest and registries the importer is given,\n" +
  "// because a browser cannot walk the content files to build them.\n" +
  "window.MarginalImportData = " + JSON.stringify({
    manifest: require("./tools/contract/libraries.js").manifest(),
    directives: require("./tools/contract/directives.js").registry(),
    questions: require("./tools/contract/libraries.js").questionRegistry(),
  }) + ";\n";
let importerOut = importerShell;
importerOut = inlineScript(importerOut, '<script src="contract-bundle.js"></script>', contractBundle);
importerOut = inlineScript(importerOut, '<script src="importer-data.js"></script>', importerData);
importerOut = inlineScript(importerOut, '<script src="importer.js"></script>', read("importer.js"));
// The importer must not be able to write. Slice 1 has no store, and the check is
// here rather than in a comment: a build that produced an importer carrying a
// persistence call would be shipping a write path nobody reviewed.
[["localStorage", /\blocalStorage\b/], ["sessionStorage", /\bsessionStorage\b/],
 ["indexedDB", /\bindexedDB\b/i], ["fetch", /\bfetch\s*\(/], ["XMLHttpRequest", /\bXMLHttpRequest\b/],
 ["sendBeacon", /\bsendBeacon\b/]].forEach(function (pair) {
  if (pair[1].test(read("importer.js")))
    throw new Error("BUILD REFUSED: importer.js reaches for " + pair[0] +
      ", and the read only importer has no persistence or network path");
});
...
const artefacts = [
  ...
  { dir: root, name: "marginal-importer.html", text: importerOut },
...
console.log(`built marginal-importer.html (${importerOut.length} characters)`);
