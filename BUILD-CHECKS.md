...
11. **Reproduce before assuming a regression (recurred).** When something "looks
    broken", find the actual cause before changing code — confirm it's a
    regression, not a device/environment issue or a feature that never existed.
    Instances: the smartboard flip "snap" was a device 3D-transform limit, not a
    code regression; the "marking explanation button" that "disappeared" had
    never existed. Pushing a fix for an unconfirmed cause is how a one-line change
    becomes three review rounds.
...
- `npm run lint:css` passes (stylelint on index.html's `<style>` block; catches
  the #7 prefix-duplication drop and shorthand overrides).
...
- **Client cache vs cloud write (seen once — watch for a second).** After a cloud
  write/import succeeds, re-hydrate the in-memory/UI cache; don't trust the
  backend write alone to update the screen. Instance: a freshly imported set saved
  to Supabase but didn't show in "Your sets" until reload. Promote to a numbered
  check if it recurs.
